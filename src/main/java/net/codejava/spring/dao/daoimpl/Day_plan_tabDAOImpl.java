...
public class Day_plan_tabDAOImpl implements Day_plan_tabDAO {
	...
	public void update(Day_plan_tab day_plan_tab) {
		...
		if (i!=0) {
			// update
			String sql = "UPDATE day_plan_tab SET plan_date=?,edit_time=?,plan_quan= ?,day_plan_recorder_num= ?,plan_status=?"+
 " WHERE day_plan_num= ?";
					//	+ "plan_time=?,plan_quan=?,plan_work_time=?,produce_plan_recorder_num=?WHERE produce_plan_num=?";
			jdbcTemplate.update(sql, day_plan_tab.getPlan_date(),day_plan_tab.getEdit_time(),day_plan_tab.getPlan_quan(),day_plan_tab.getDay_plan_recorder_num(),day_plan_tab.getPlan_status(),day_plan_tab.getDay_plan_num());
