	...
	//折线图、柱状图数据
	 @RequestMapping(value="/energylinedata")
		public @ResponseBody EchartData gasline() throws IOException{ 
		       ...
		       user_select_equip[0]= "DCC800";
		       user_select_equip[1]= "DCC900";
		       
		       String start_time=new String("2015-11-09 2:30:24");
               ...
               for (j=0;j<equip_serie_num;j++){
            	  ...
            	  // System.out.println("j是"+Integer.toString(j));
            	   ...
            	   Ener_stat_tab_for_a_equip = ener_stat_tabDAO.GetSpecData(user_select_equip[j],start_time,end_time,"electric");
	 ...
	 @RequestMapping(value="/energypiedata")
		public @ResponseBody EchartData echartspie() throws IOException{ 
		 ...
		 int equip_serie_num=2;
		   ...
		   List<Series> series = new ArrayList<Series>();
		   series.add(new Series("所选设备电能消耗比例","pie",total_energy,equip_label));
	        //添加第一个饼图的数据，第一项不用填，第二项为类型，第三项为序列的值，第四项是数据分组，注意第三项和第四项的顺序是对应的
	        //返回数据
	        EchartData data=new EchartData(legend, category, series);  
